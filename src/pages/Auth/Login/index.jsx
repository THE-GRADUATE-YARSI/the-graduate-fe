  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (decodedToken.role === "student") {
        ...
        navigate("/dosen/dashboard-dosen");
      } else {
        navigate("/");
      }
    } catch (error) {
      ...
      <div className="bg-white border-t-4 border-blue-600 rounded-b text-black px-4 py-3 shadow-md rounded-md w-80">
        ...
        <form onSubmit={handleSubmit}>
          ...
          <Link
              to="/forgot-password"
              type="submit"
              className="py-2 w-full rounded-sm text-center text-sky-500 hover:text-sky-800 font-medium text-sm"
              >
              Lupa Password
            </Link>
              </div>
          <div className="flex items-center justify-between gap-3 mt-3">
            <Link
              to="/register"
              type="submit"
              className="py-2 w-full rounded-sm bg-[#28a745] hover:bg-green-700 text-center text-white font-medium text-sm"
            >
              Register
            </Link>
            <button
